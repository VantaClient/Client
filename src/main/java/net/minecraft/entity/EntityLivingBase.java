...
public abstract class EntityLivingBase extends Entity {
    ...
    public void onLivingUpdate() {
        ...
        EntityMotionEvent event = new EntityMotionEvent(0.005D);
        event.call();
        if (Math.abs(this.motionX) < event.motion) {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < event.motion) {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < event.motion) {
            this.motionZ = 0.0D;
        }
