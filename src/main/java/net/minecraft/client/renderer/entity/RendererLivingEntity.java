...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    protected float interpolateRotation(float previous, float current, float partialTicks) {
        float f;

        for (f = current - previous; f < -180.0F; f += 360.0F) {
        }
        ...
        return previous + partialTicks * f;
    }
