...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private void setWindowIcon() {
        ...
        if (util$enumos != Util.EnumOS.OSX) {
            ...
            InputStream x64 = null;

            try {
                x32 = Vanta.class.getResourceAsStream("/assets/vanta/icon/32x32.png");
                x64 = Vanta.class.getResourceAsStream("/assets/vanta/icon/64x64.png");

                if (x32 != null && x64 != null) {
                    Display.setIcon(new ByteBuffer[]{this.readImageToBuffer(x32), this.readImageToBuffer(x64)});
                }
            } catch (IOException ioexception) {
                logger.error("Couldn't set icon", ioexception);
            } finally {
                IOUtils.closeQuietly(x32);
                IOUtils.closeQuietly(x64);
            }
        }
    }
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ...
        DisplayGuiScreenEvent displayGuiScreenEvent = new DisplayGuiScreenEvent(guiScreenIn);
        displayGuiScreenEvent.call();

        if (displayGuiScreenEvent.screen == null && this.thePlayer.getHealth() <= 0.0F) {
            displayGuiScreenEvent.screen = new GuiGameOver();
        }

        this.currentScreen = displayGuiScreenEvent.screen;

        if (displayGuiScreenEvent.screen != null) {
            this.setIngameNotInFocus();
            ScaledResolution scaledresolution = new ScaledResolution(this);
            int i = scaledresolution.getScaledWidth();
            int j = scaledresolution.getScaledHeight();
            displayGuiScreenEvent.screen.setWorldAndResolution(this, i, j);
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            ClickEvent clickProcessingEvent = new ClickEvent();
            clickProcessingEvent.call();
        ...
        if (ViaLoadingBase.getInstance() != null && ViaLoadingBase.getInstance().getTargetVersion().newerThanOrEqualTo(ProtocolVersion.v1_21_2)) {
